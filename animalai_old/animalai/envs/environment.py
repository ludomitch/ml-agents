...
class UnityEnvironment(BaseUnityEnvironment):
    ...
    def __init__(
        ...
        n_arenas: int = 1,
        play: bool = False,
        arenas_configurations: List[ArenaConfig] = None,
        ...
        if file_name is not None:
            ...
            logger.info(
                "Launch the environment container (or Play button in the Unity Editor)."
            )
        self._loaded = True
        ...
        if self._unity_version != self._version_:
            ...
            raise UnityEnvironmentException(
                ...
                "Please go to https://github.com/beyretb/AnimalAI-Olympics to download the latest version ".format(
                    self._version_, self._unity_version
                )
            )
        self._n_agents = {}
    ...
    @staticmethod
    def get_communicator(worker_id, base_port):
    ...
    def executable_launcher(self, file_name, docker_training):
        ...
        if launch_string is None:
            ...
            raise UnityEnvironmentException(
                ...
                "https://github.com/beyretb/AnimalAI-Olympics ".format(true_filename)
            )
        else:
            ...
            if not docker_training:
                ...
                if self.resolution:
                    docker_ls = (
                        ...
                        " {0} --port {1} --nArenas {2} --resolution {3}"
                    ).format(
                        launch_string,
                        str(self.port),
                        str(self.n_arenas),
                    ...
                    docker_ls = (
                        ...
                        " --server-args='-screen 0 640x480x24'"
                        " {0} --port {1} --nArenas {2}"
                    ).format(launch_string, str(self.port), str(self.n_arenas))
                self.proc1 = subprocess.Popen(
                    docker_ls,
    ...
    def __str__(self):
        ...
        Number of Brains: {1}
        Number of Training Brains : {2}""".format(
            self._academy_name, str(self._num_brains), str(self._num_external_brains)
        )
    ...
    def _generate_step_input(
        self, vector_action, memory, text_action, value
    ) -> UnityRLInput:
        rl_in = UnityRLInput()
    ...
    def _generate_reset_input(
        self, training, config: ArenaConfig, seed: int
    ) -> UnityRLInput:
        rl_in = UnityRLInput()
