...
class AnimalAIEnv(gym.Env):
    ...
    def __init__(
        ...
        initial_info = self._env.reset(arenas_configurations=arenas_configurations)[
            self.brain_name
        ]
        ...
        if self.retro and self._n_agents > 1:
            raise UnityGymException(
                "Only one agent is allowed in retro mode, set n_agents to 1."
            )
    ...
    def reset(self, arenas_configurations=None, seed=-1):
        ...
        Returns: observation (object/list): the initial observation of the
            space.
        """
        info = self._env.reset(arenas_configurations=arenas_configurations, seed=seed)[
            self.brain_name
        ]
    ...
    def _greyscale_obs_single(self, obs):
        new_obs = (
            np.floor(np.expand_dims(np.mean(obs, axis=2), axis=2))
            .squeeze()
            .astype(np.uint8)
        )
        return new_obs
    ...
    def _greyscale_obs_multi(self, obs):
        new_obs = [
            np.floor(np.expand_dims(np.mean(o, axis=2), axis=2))
            .squeeze()
            .astype(np.uint8)
            for o in obs
        ]
